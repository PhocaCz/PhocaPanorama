...
if ($this->t['search']) {
	echo '<div class="alert alert-message">' . JText::_('COM_PHOCAPANORAMA_SEARCH_FILTER_IS_ACTIVE') .'</div>';
}
...
echo $r->selectFilterPublished('JOPTION_SELECT_PUBLISHED', $this->state->get('filter.state'));
echo $r->selectFilterLanguage('JOPTION_SELECT_LANGUAGE', $this->state->get('filter.language'));
echo $r->selectFilterLevels('COM_PHOCAPANORAMA_SELECT_MAX_LEVELS', $this->state->get('filter.level'));
echo $r->endFilterBar();
...
if (isset($item->image) && JFile::exists(JPATH_ROOT .'/'.$item->image)) {
	echo '<img src="'.JURI::root().$item->image.'" style="width: 50px;height:auto;" />';
}
echo '</td>';
